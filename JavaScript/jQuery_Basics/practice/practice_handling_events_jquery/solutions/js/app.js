...
$('#addNew').on('click', function() {
  const newRestaurantInput = $('#newRestaurantInput').val();
  ...
  const newLi = $('ul').append(`<li>${newRestaurantInput}</li>`);
  // BONUS: clear the value
  $('#newRestaurantInput').val('');
});
// or if pressing enter after typing a name
$('#newRestaurantInput').keypress(function(event){
    if(event.which == 13){//Enter key pressed
        $('#addNew').click();//Trigger addNew button click event
    }
});
